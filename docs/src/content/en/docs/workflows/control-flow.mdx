...
### Handling failures with `allSettled` mode

By default, `.parallel()` uses `'all'` mode, which fails the entire parallel block if any step throws an error. Use `{ mode: 'allSettled' }` to allow the workflow to continue even when individual steps fail. Failed steps are excluded from the output, while successful steps' results are preserved.

This is useful when running steps that may independently fail — for example, multiple research agents where some may encounter expired auth tokens or unavailable services — and you want to continue with whatever results are available.
...
const researcherA = createStep({
  ...
  execute: async () => ({ brief: 'GitHub activity found' }),
})
...
const researcherC = createStep({
  ...
  inputSchema: z.any(),
  outputSchema: z.object({ brief: z.string() }),
  execute: async () => ({ brief: 'Slack messages found' }),
})
...
const writerStep = createStep({
  ...
  execute: async ({ inputData }) => {
    // inputData contains only successful results:
    // { 'researcher-a': { brief: '...' }, 'researcher-c': { brief: '...' } }
    // 'researcher-b' is missing because it failed
    const briefs = Object.values(inputData)
      .filter((v: any) => v && typeof v === 'object' && 'brief' in v)
      .map((v: any) => v.brief)
...
- `'all'` mode (default): any step failure fails the entire parallel block
- `'allSettled'` mode: the block succeeds even if some or all steps fail
- Failed steps are excluded from the output object — only successful steps appear as keys
- Downstream steps should handle potentially missing keys from failed steps
...
| `.parallel([a, b])` | Different operations on same input | `T` | `{ a: U, b: V }` | All run simultaneously (supports `allSettled` mode) |
